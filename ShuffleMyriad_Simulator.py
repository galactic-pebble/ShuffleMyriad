...
class ShuffleMyriadApp:
    def __init__(self, root):
        ...
        self.chip_picker_window_instance = None
    ...
    def _setup_ui_elements(self):
        ...
        main_buttons_config = [
            ...
            ("チップを追加", self.open_chip_picker_window),
    ...
    def add_chip(self, color_name, label=""):
        chip_size = 36
        marker = {
            "type": "chip",
    ...
    def open_chip_picker_window(self):
        if self.chip_picker_window_instance is None or not self.chip_picker_window_instance.is_active():
            self.chip_picker_window_instance = ChipPickerWindow(self)
        else:
            self.chip_picker_window_instance.lift_window()
...
class ChipPickerWindow:
    def __init__(self, app_ref):
        self.app = app_ref
        self.root = app_ref.root
        self.window = tk.Toplevel(self.root)
        self.window.title("チップを追加")
        center_tk_window(self.root, self.window, 320, 220)
        self.window.protocol("WM_DELETE_WINDOW", self.destroy_window)

        frame = tk.Frame(self.window, padx=10, pady=10)
        frame.pack(fill="both", expand=True)

        color_chips = [("赤", "red"), ("青", "blue"), ("黄", "yellow"), ("緑", "green"), ("白", "white")]
        number_chips = ["1", "2", "3", "4", "5"]

        tk.Label(frame, text="色チップ").pack(anchor="w")
        color_frame = tk.Frame(frame)
        color_frame.pack(fill="x", pady=(4, 10))
        for i, (label, color_name) in enumerate(color_chips):
            tk.Button(color_frame, text=label, width=5, command=lambda c=color_name: self._add_and_close(c, "")).grid(row=0, column=i, padx=2, pady=2)

        tk.Label(frame, text="数字チップ").pack(anchor="w")
        number_frame = tk.Frame(frame)
        number_frame.pack(fill="x", pady=(4, 10))
        for i, number in enumerate(number_chips):
            tk.Button(number_frame, text=number, width=5, command=lambda n=number: self._add_and_close("white", n)).grid(row=0, column=i, padx=2, pady=2)

    def _add_and_close(self, color_name, label):
        self.app.add_chip(color_name, label)
        self.destroy_window()
    ...
    def destroy_window(self):
        if self.window:
            self.window.destroy()
            self.window = None
        self.app.chip_picker_window_instance = None
